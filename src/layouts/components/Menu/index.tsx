...
import { getRoutersInfo } from "@/utils/session";
...
const LayoutMenu = (props: any) => {
	...
	const deepLoopFloat = (menuList: Menu.MenuOptions[], newArr: MenuItem[] = []) => {
		menuList.forEach((item: Menu.MenuOptions) => {
			...
			if (!item?.children?.length) return newArr.push(getItem(item.menuName, item.path, addIcon(item.icon!)));
			newArr.push(getItem(item.menuName, item.path, addIcon(item.icon!), deepLoopFloat(item.children)));
		});
		return newArr;
	};
	...
	const getMenuData = async () => {
		setLoading(true);
		try {
			const data = await getRoutersInfo();
			if (!data) return;
