...
const PicSearcher: React.FC = () => {
	...
	return (
		<div className="iconPicSearcher">
			<Popover content="AI 截图搜索上线了，快来体验吧！🎉" open={state.popoverVisible}>
				<AntdIcons.CameraOutlined className="icon-pic-btn" onClick={toggleModal} />
			</Popover>
			<Modal title="信息" open={state.modalOpen} onCancel={toggleModal} footer={null}>
				{state.modelLoaded || (
				...
				{state.modelLoaded && (
					...
					>
						...
						<p className="ant-upload-hint">我们会通过上传的图片进行匹配，得到最相似的图标</p>
					</Dragger>
				)}
				<Spin spinning={state.loading} tip="匹配中...">
					<div className="icon-pic-search-result">
						{state.icons.length > 0 && <div className="result-tip">为您匹配到以下图标：</div>}
						<table>
						...
						{state.error && <Result status="500" title="503" subTitle="识别服务暂不可用" />}
					</div>
				</Spin>
			</Modal>
		</div>
