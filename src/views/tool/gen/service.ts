...
export async function getGenCodeList(params?: GenCodeTableListParams) {
	...
	return request({
		url: `/api/tool/gen/list?${queryString}`,
		data: params,
		method: "get",
		headers: {
...
export async function queryTableList(params?: any) {
	const queryString = new URLSearchParams(params).toString();
	return request({
		url: `/api/tool/gen/db/list?${queryString}`,
		data: params,
		method: "get",
		headers: {
			"Content-Type": "application/json;charset=UTF-8"
...
export async function importTables(tables?: string) {
	return request({
		url: `/api/tool/gen/importTable?tables=${tables}`,
		method: "POST",
		headers: {
			"Content-Type": "application/json;charset=UTF-8"
		}
