 ...
 */
export function buildTreeData(data: any[], id: string, name: string, parentId: string, parentName: string, children: string) {
	const config = {
		id: id || "key",
		name: name || "name",
	...
	data.forEach(item => {
		...
		d.value = d[config.id];
		d[config.childrenList] = d.children;
		nodeIds[d[config.id]] = d;
		childrenListMap[pId].push(d);
		// if (d.children && Array.isArray(d.children)) {
		// 	return buildTreeData(d.children, "menuId", "menuName", "", "", "");
		// }
	});
