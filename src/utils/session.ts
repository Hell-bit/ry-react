...
// 			}
// 			const newRoute = {
// 				element: React.createElement(lazy(() => import("@/views/" + path))),
...
export async function getRouters(): Promise<any> {
	return request({ url: "/api/getRouters" });
}

export function convertCompatRouters(childrens: API.RoutersMenuItem[]): any[] {
	return childrens.map((item: API.RoutersMenuItem) => {
		if (item.path == "/" && item.children) return convertCompatRouters(item.children)[0];
		return {
			path: item.path,
			icon: item.meta.icon,
			//  icon: item.meta.icon,
